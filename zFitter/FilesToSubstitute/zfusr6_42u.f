*==========================================================================*
*                                                                          *
* This is ZFITTER version 6.42, March 18th, 2005 (Dubna--Zeuthen version)  *
*                                                                          *
* program and description are obtainable from:                             *
* http://www.ifh.de/~riemann/Zfitter/zf.html                               *
* recent versions of programs are obtainable from:                         *
* /afs/cern.ch/user/b/bardindy/public/ZF6_42 etc.                          *
*                                                                          *
* SUBROUTINES:  ZFTEST, ZUINIT, ZUINFO, ZUFLAG, ZUWEAK, ZUCUTS,            *
* ------------  ZUTHSM, ZUATSM, ZUTPSM, ZULRSM, ZDIZET, ZQCDCO,            *
*               ZUXSEC, ZUXSA , ZUXSA2, ZUTAU,  ZUALR                      *
*==========================================================================*
*                                                                          *
...
*
      ...
      PARAMETER(ZMASS=91.1876D0,TMASS=178.0D0,HMASS=100.D0)
      PARAMETER(ALFAS=.117D0)
...
C      PARAMETER(QE=2.D0/3.D0,AE=.5D0,QU= 2.D0/3.D0,AU= .5D0,
     +                            QD=-1.D0/3.D0,AD=-.5D0)
*
...
*
      CALL ZUCUTS(11,0,15.D0,10.D0,0.D0,ANG0,ANG1,0.D0)
      CALL ZUINFO(0)
*
...
*     
* define the output
      OPEN(UNIT = 55,FILE="PolarizationAndXsecQuarkU_$replacement_sin2theta_label.dat")
...
C        IF(I.EQ.5) RS=60D0
C        IF(I.EQ.6) RS=65D0
C        IF(I.EQ.7) RS=ZMASS-2D0
C        IF(I.EQ.8) RS=ZMASS-1D0
C        IF(I.EQ.9) RS=ZMASS
C        IF(I.EQ.10) RS=ZMASS+1D0
C        IF(I.EQ.11) RS=ZMASS+2D0
C        IF(I.EQ.12) RS=100D0
C        IF(I.EQ.13) RS=140D0
C        IF(I.EQ.14) RS=175D0
...
C         IF(INDF.EQ.3) CALL ZUTAU(RS,ZMASS,GAMZ,0,GVE,GAE,GVF,GAF,
C         IF(INDF.EQ.3) THEN
C            GAF = 0.501408713
C            GVF = 0.201408713
C         ENDIF
     ...
     +     TAUPOL(2),TAUAFB(2))
           IF(INDF.EQ.3) THEN 
              ...
              PRINT *,' ZUTAUGAE = ',GAE
              PRINT *,' ZUTAUGVE = ',GVE
              WRITE(55,"(1X,6F10.5)") RS, XS(INDF,3),TAUPOL(2)
           ENDIF
...
*
      ...
      U2    = Q2-S
*
* NEUTRINO CHANNELS INITIALIZATION
*
      QF       = 0.D0
      ALLCH(0) = QF
      ALLMS(0) = 0.D0
      ...
      DO 3 I = 1,4
 3    XALLCH(1,I) = XFZ(I)
      S2TEFF(0) = SIN2TW*DREAL(XFZ(3))
*
* LEPTON   CHANNELS INITIALIZATION
*
      QF=-1D0
      DO 4 I = 1,3
 4    ALLCH(I) = QF
      ...
      DO 5 I = 1,4
 5    XALLCH(2,I) = XFZ(I)
      S2TEFF(1) = SIN2TW*DREAL(XFZ(3))
      S2TEFF(2) = SIN2TW*DREAL(XFZ(3))
      S2TEFF(3) = SIN2TW*DREAL(XFZ(3))
      PRINT *,'S2TEFF(3) = ',S2TEFF(3)
*
...
*
      ...
      NPARD(2) = IFLAGS(IFAMT4)
      NPARD(3) = IFLAGS(IFQCDC)
      NPARD(4) = 1
      NPARD(5) = IMASS
