...
*
      ...
      AROTFZ(INF-1)=ROFACI
      ARKAFZ(INF-1)=AKFACI
      SINEFF=$replacement_sin2theta
      ARVEFZ(INF-1)=1D0-4D0*SINEFF*AQFI(INF)
C      ARVEFZ(INF-1)=1D0-4D0*SW2M*AKFACI*AQFI(INF)
      PRINT *,' dizetSW2M = ',SW2M
      PRINT *,' dizetAKFAC = ',AKFAC
      PRINT *,' INF = ',INF
      PRINT *,' AQFI(INF-1) = ',AQFI(INF-1)
      PRINT *,' ARVEFZ(INF-1) = ',ARVEFZ(INF-1)
      ARSEFZ(INF-1)=SINEFF
      AIROFZ(INF-1)=AR1IM
      AIKAFZ(INF-1)=AK1IM
